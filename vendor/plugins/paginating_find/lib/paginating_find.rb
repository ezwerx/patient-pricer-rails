...
module PaginatingFind
  ...
  module ClassMethods
    ...
    def paginating_ferret_search(options, find_options)
       count = find_by_contents(options[:q], {:lazy => false}).total_hits
...
#          logger.warn([':find_options = ',find_options])          
#          res = find_by_contents(options[:q], {:offset => offset, :limit => limit}, options[:find_options])
          res = find_by_contents(options[:q], {:sort => options[:sort], :lazy => false, :offset => offset, :limit => limit, :filter_proc => options[:filter_proc]}, find_options)
        end
    end

    def paginating_ferret_multi_search(options, find_options)
       count = multi_search(options[:q], options[:additional_models], {:limit => :all, :lazy => true}).total_hits
       PagingEnumerator.new(options[:page_size], count, false, options[:current], 1) do |page|
          offset = (options[:current].to_i - 1) * options[:page_size]
          offset = 0 if offset < 0
          limit = options[:page_size]

          multi_search(options[:q], options[:additional_models], {:sort => options[:sort], :offset => offset, :limit => limit, :filter_proc => options[:filter_proc]}, find_options)
        end
    end    
  end
end